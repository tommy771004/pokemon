...
const TYPES = [
  { key: "stairs", en: "Stairs & Access", zh: "階梯與通道" },
  { key: "door", en: "Doors", zh: "門" },
  { key: "window", en: "Windows", zh: "窗戶" },
  { key: "roof", en: "Roofs", zh: "屋頂" },
  { key: "wall", en: "Walls & Partitions", zh: "牆與隔板" },
  { key: "fence", en: "Fences", zh: "柵欄與圍欄" },
  { key: "pillar", en: "Pillars & Beams", zh: "柱與骨架" },
  { key: "path", en: "Paths & Bridges", zh: "通道與橋樑" },
  { key: "pipe", en: "Pipes", zh: "管線" },
  { key: "edge", en: "Edges & Borders", zh: "邊框" },
...
export default function Buildings() {
  ...
  }, [query, selectedType, showFavoritesOnly]);
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((item) => {
      const nameCurrent = en ? item.nameEn || item.name : item.name || item.nameEn;
      const matchesQuery =
        !q || nameCurrent?.toLowerCase().includes(q) || item.id?.includes(q);
      const matchesType = !selectedType || item.buildingType === selectedType;
      const matchesFavorites = !showFavoritesOnly || favorites.has(item.id);
      return matchesQuery && matchesType && matchesFavorites;
    });
  }, [data, query, selectedType, en, showFavoritesOnly, favorites]);
  ...
  const hasActiveFilters = Boolean(selectedType || query || showFavoritesOnly);
            ...
            >
              ...
              {hasActiveFilters && (
                <span className="font-mono-metadata text-[10px] bg-primary text-white px-1.5 py-0.5 rounded-full ml-1 leading-none">
                  {(selectedType ? 1 : 0) + (query ? 1 : 0) + (showFavoritesOnly ? 1 : 0)}
                </span>
              )}
            </button>
          </div>
          ...
          <div className="sticky top-0 md:top-[120px]">
            ...
            <div className="mb-lg border bg-surface-container/20 rounded-sm">
              <h3 className="font-label-caps text-label-caps text-ink-soft border-line-soft p-3 flex items-center justify-between">
                <span className="flex items-center gap-2">
                  ...
                  {en ? "Type" : "建築分類"}
                  {selectedType && (
                    <span className="font-mono-metadata text-[10px] bg-primary text-white px-2 py-0.5 rounded-full ml-2 leading-none">
                      1
                    </span>
                  )}
                </span>
              ...
              <div className="p-3 pt-0 border-t border-line-soft/50 space-y-xs">
                <button
                  ...
                  className={`w-full text-left font-body-base text-body-base flex items-center justify-between py-xs transition-colors group p-2 rounded ${selectedType === null ? "text-primary bg-primary/5" : "text-ink-soft hover:text-ink-main hover:bg-surface-container-high"}`}
                >
                  <span className="flex items-center gap-xs">
                    <span
                      className={`w-2 h-2 rounded-full transition-colors ${selectedType === null ? "bg-primary" : "bg-ink-faint group-hover:bg-primary"}`}
                    ></span>
                    {en ? "All Types" : "不限分類"}
                  </span>
                </button>
                {TYPES.map((type) => (
                  <button
                    ...
                    className={`w-full text-left font-body-base text-body-base flex items-center justify-between py-xs transition-colors group p-2 rounded ${selectedType === type.key ? "text-primary bg-primary/5" : "text-ink-soft hover:text-primary hover:bg-surface-container-high"}`}
                  >
                    <span className="flex items-center gap-xs">
                      <span
                        className={`w-2 h-2 rounded-full transition-colors ${selectedType === type.key ? "bg-primary" : "bg-tertiary group-hover:bg-primary"}`}
                      ></span>
                      {en ? type.en : type.zh}
                    </span>
                  </button>
            ...
            <div className="mb-lg border bg-surface-container/20 rounded-sm">
              ...
              <div className="p-3 pt-0 border-t border-line-soft/50">
                ...
                >
                  <span className={`material-symbols-outlined text-[14px] ${showFavoritesOnly ? "fill-white font-variation-fill" : ""}`}>
                    favorite
                  </span>
          ...
          {visible.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-md relative">
              {visible.map((item: any, idx: number) => (
                <ScrollFade key={item.id} depth="none" delay={(idx % 5) * 0.03} className="h-full">
                  <div
                      ...
                      >
                        <div className="flex justify-between items-start mb-2 sm:mb-sm">
                          <span className="font-body-italic text-sm sm:text-base text-ink-soft line-clamp-1">
                            {TYPES.find((tp) => tp.key === item.buildingType)?.[en ? "en" : "zh"] ||
                              (en ? "Building" : "建築")}
                          </span>
                          <div className="flex items-center gap-2">
                            <button
                        ...
                        <div className="w-full h-24 sm:h-32 mb-4 relative flex items-center justify-center group-hover/front:scale-105 transition-transform duration-500">
                          <img
                            ...
                            alt={en ? item.nameEn || item.name : item.name}
                            loading="lazy"
                        ...
                        <div className="flex flex-col mt-auto pt-2 border-t border-line-soft">
                          <h2 className="font-headline-sm md:font-headline-md text-headline-sm md:text-headline-md mb-1 transition-colors truncate text-ink-soft group-hover/front:text-ink-main">
                            {en ? item.nameEn || item.name : item.name}
                          </h2>
                        </div>
                      </article>
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          {item.recipe && item.recipe.length > 0 && (
                            <div className="pt-2 border-t border-line-soft/50">
                              ...
                              <ul className="pl-3 space-y-0.5">
                                {item.recipe.map((note: string, i: number) => (
                                  <li
                                    key={i}
                          ...
                          {item.obtain && item.obtain.length > 0 && (
                            <div className="pt-2 border-t border-line-soft/50">
                              ...
                              <ul className="pl-3 space-y-0.5">
                                {item.obtain.map((method: string, i: number) => (
                                  <li
                                    key={i}
                                    className="font-body-base text-[11px] text-ink-soft list-disc"
                                  >
